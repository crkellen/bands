...
socket.on('init', (data) => {
  ...
  for( let i = 0; i < data.player.length; i++ ) {
    ...
    cGame.cPlayers.push( new cPlayer(data.player[i]) );
    console.info(cGame.cPlayers);
  }
...
var drawGrid = () => {
  ...
  cGame.ctx.drawImage(Imgs.grid, 0, 0);
};

var drawEntities = () => {
  //Each player object draws itself
  for( let p in cGame.cPlayers ) {
    console.info(cGame.cPlayers[p].drawSelf());
    //cGame.cPlayers[p].drawSelf(); //#FIXME: .drawSelf causes cGame to be undefined
  }
  //Each bullet object draws itself
  for( let b in cGame.cBullets ) {
    //cGame.cBullets[b].drawSelf(); //#FIXME: cGame is undefined
  }
};
